...
const string strMessageMagic = "FastMine Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight < 1) // no block reward within the first 3 days
        nSubsidy = 0;
    if(nHeight > 105192000) // no block reward after 50 years
        nSubsidy = 0;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60 * 60; // FastMine: 1 Hour
static const int64 nTargetSpacing = 10; // FastMine: 10 seconds
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal fastmine miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FastMine", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
	...
	// vMerkleTree: 5a2e19825b
        ...
        // Genesis block
        const char* pszTimestamp = "US blizzard 2016: New York lifts travel ban as clear-up begins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
